...
module.exports = function (io, socket, db, state, rateLimiter) {
  ...
  // Game: spectate
  socket.on('game:spectate', function (data) {
  ...
  socket.on('game:active-list', function () {
    socket.emit('game:active-list', state.games);
  });

  // Game: spectate list (for sidebar)
  socket.on('game:spectate:list', function () {
  ...
  // Battle: round action
  socket.on('battle:round-action', function (data) {
  ...
  // Battle: score update
  socket.on('battle:score', function (data) {
  ...
  // Battle: end
  socket.on('battle:end', function (data) {
    if (!data || !data.id) return;
    for (var i = 0; i < state.battles.length; i++) {
      if (state.battles[i].id === data.id) {
